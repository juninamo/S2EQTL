...
association_nam = function(seurat_object=NULL,
  ...
  .reg <- function(q, k) {
    Xpc <- U[, 1:k] # XpcはUの最初のk列を取り出した行列で、k個の主成分に対応します。
    beta <- t(Xpc) %*% q # 主成分空間におけるqの重回帰係数（beta）を計算しています。## 左から両方にXpcをかけると理解できる：Xpc %*% beta = y
    qhat <- Xpc %*% beta # 主成分空間におけるqの予測値（qhat）を計算しています。
    return(list(qhat = qhat, beta = beta))
  }
  
  .stats <- function(yhat, ycond, k) {
    ssefull <- crossprod(yhat - ycond) # 予測値と実際の値の差（予測誤差）の二乗和（Sum of Squares for Error, SSE）を計算しています。これはフルモデル（全ての説明変数を用いたモデル）の予測誤差です。
    ssered <- crossprod(ycond) # 実際の値の二乗和を計算しています。これは、説明変数が一つもないモデル（すなわち、平均値だけで予測するモデル）における予測誤差です。
    deltasse <-  ssered - ssefull # 説明変数が一つもないモデルの予測誤差からフルモデルの予測誤差を引いています。これにより、モデルによる説明により減少した誤差の量を計算しています。
    f <- (deltasse / k) / (ssefull/n) # F統計量を計算しています。F統計量は、モデルがデータにどれだけ適合しているかを評価するための統計量です。
    p <- -pf(f, k, n-(1+r+k), log.p = TRUE)    # F統計量に対応するp値を計算しています。p値はモデルの有意性を評価するための統計量で、p値が小さいほどモデルが有意にデータを説明していると判断します。
    r2 <- 1 - ssefull/ssered # 決定係数（R^2）を計算しています。決定係数は、全体の変動のうちモデルが説明できる割合を示します。
    return(list(p=p, r2=r2))
  }
  
  .minp_stats <- function(z) {
    zcond <- scale(M %*% z, center = FALSE, scale = TRUE) # z（おそらく観測値）が説明変数Mによって予測された値にスケーリング（正規化）されています。
    qhats <- purrr::map(ks, function(k) .reg(zcond, k)$qhat) # ksというリストに含まれる各主成分数について、zcondに対する主成分回帰を行っています。.reg(zcond, k)$qhatは主成分回帰による予測値を返します。これを全ての主成分数kに対して行い、結果の予測値のリストをqhatsとします。
    .tmp <- purrr::map2(qhats, ks, function(qhat, k) .stats(qhat, zcond, k)) # qhatsとksを用いて、各主成分数kに対するモデルの統計的評価を行います。これは、.stats(qhat, zcond, k)によって行われ、これにより各kに対するF統計量のp値と決定係数を計算します。そして、それぞれのp値のリストpsと決定係数のリストr2sを得ます。
    ps <- purrr::map_dbl(.tmp, 'p') # 
    r2s <- purrr::map_dbl(.tmp, 'r2')
    k_ <- which.min(ps) # which.min(ps)によって最小のp値を持つ主成分数のインデックスを取得し、それに対応する主成分数ks[k_]、p値ps[k_]、および決定係数r2s[k_]を返しています。
    return(list(k=ks[k_], p=ps[k_], r2=r2s[k_]))
  }
  
  ## このコードは、上記の.minp_stats関数と.reg関数を使用して、最適な主成分数で回帰モデルを適用し、モデルの結果を評価するものです。具体的には以下の手順を行っています。
  # get non-null f-test p-value
  .tmp <- .minp_stats(y)　## .minp_stats(y)を使って、観測データyに対する最適な主成分数（k）を決定します。この最適な主成分数は、ksの中で最小のp値をもたらすものです。さらに、このモデルのp値（p）と決定係数（r2）を保存します。
  k <- .tmp$k
  p <- .tmp$p
  r2 <- .tmp$r2
  if (k == max(ks)) {　## 最適な主成分数が設定できる最大値（max(ks)）と等しい場合、ユーザーに警告メッセージを表示します。これは、設定できる主成分数が不足している可能性を示すためです。
  ...
  ycond <- scale(M %*% y, center = FALSE, scale = TRUE)　## 次に、観測データyを説明変数行列Mを用いて正規化します。
  .tmp <- .reg(ycond, k) ## この正規化されたデータycondに対して、最適な主成分数kで.reg関数を適用します。その結果、回帰予測値（yhat）と回帰係数（beta）を得ます。
  yhat <- .tmp$qhat
  beta <- .tmp$beta
  r2_perpc <- (beta / as.numeric(sqrt(crossprod(ycond))))**2　## 最後に、それぞれの主成分の寄与度（r2_perpc）を計算します。これは、各主成分の回帰係数を正規化された観測データycondの長さで割り、2乗したものです。この値は0から1の範囲をとり、各主成分が目的変数の分散をどの程度説明するかを示しています。
  ## 以上の手順により、観測データyに対する最適な主成分回帰モデルを構築し、そのモデルの結果を評価するという操作を行っています。
...
NAM_NMF_GP = function(seurat_object=NULL,
  ...
  p <- pred(fit, form_df, 
            reduce = mean,
            verbose = FALSE) # use posterior mean kernel parameters
  # set.seed(seed)
  ...
  res <- list(prediction = p_cell, ## Association results 
              model = fit,
